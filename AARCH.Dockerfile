...
RUN apt update \
    ...
    && curl -o /opt/Jackett.Binaries.LinuxMuslARM64.tar.gz -L "https://github.com/Jackett/Jackett/releases/download/${JACKETT_VERSION}/Jackett.Binaries.LinuxMuslARM64.tar.gz" \
    && tar -xzf /opt/Jackett.Binaries.LinuxMuslARM64.tar.gz -C /opt \
    && rm -f /opt/Jackett.Binaries.LinuxMuslARM64.tar.gz \ 
    && apt purge -y \
    ca-certificates \
    curl \
    && apt-get clean \

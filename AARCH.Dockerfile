...
RUN apt update \
    ...
    && curl -o /opt/Jackett.Binaries.LinuxARM32.tar.gz -L "https://github.com/Jackett/Jackett/releases/download/${JACKETT_VERSION}/Jackett.Binaries.LinuxARM32.tar.gz" \
    && tar -xzf /opt/Jackett.Binaries.LinuxARM32.tar.gz -C /opt \
    && rm -f /opt/Jackett.Binaries.LinuxARM32.tar.gz \ 
    && apt purge -y \
    ca-certificates \
    curl \
    && apt-get clean \
